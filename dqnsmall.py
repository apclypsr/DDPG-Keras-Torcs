...
class DeepQNetwork:
    ...
    def buildNetwork(self, name, trainable, numActions):
        ...
        x = tf.placeholder(tf.uint8, shape=[None, 160, 160, 12], name="screens")
        print(x)
        ...
        with tf.variable_scope("cnn1_" + name):
            W_conv1, b_conv1 = self.makeLayerVariables([16, 16, 12, 32], trainable, "conv1")
        ...
        with tf.variable_scope("cnn2_" + name):
            W_conv2, b_conv2 = self.makeLayerVariables([8, 8, 32, 64], trainable, "conv2")
        ...
        with tf.variable_scope("cnn3_" + name):
            W_conv3, b_conv3 = self.makeLayerVariables([8, 8, 64, 64], trainable, "conv3")
        ...
        h_conv3_flat = tf.reshape(h_conv3, [-1, 8*8*64], name="h_conv4_flat")
        print(h_conv3_flat)

        # Fifth layer is fully connected with 512 relu units
        with tf.variable_scope("fc1_" + name):
            W_fc1, b_fc1 = self.makeLayerVariables([8*8*64, 512], trainable, "fc1")
            print(W_fc1)
            print(b_fc1)
